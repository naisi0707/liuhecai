...
const { login, hydrate } = useAgentAuth()
const isDev = import.meta.dev
const username = ref(isDev ? 'agent_a' : '')
const password = ref(isDev ? 'agent123' : '')
const loading = ref(false)

onMounted(() => {
  hydrate()
  // 已登录跳转由 auth.global 中间件处理，避免重复 navigate
})
...
<template>
  <div class="login-page">
    <el-card style="width:420px;">
      ...
      <p v-if="isDev" style="color:#6b7280;margin-top:12px;">演示：agent_a / agent123</p>
    </el-card>
  </div>
</template>
